...
class _TtsHomePageState extends State<TtsHomePage> {
  ...
  static const String _androidDownloadFolder = '/storage/emulated/0/Download/Adamson-tts';
  ...
  Future<Directory> _resolveDownloadsDirectory() async {
    if (Platform.isAndroid) {
      final appDownloadDir = Directory(_androidDownloadFolder);
    ...
    if (knownDownloads != null) {
      final appDownloadDir = Directory('${knownDownloads.path}/Adamson-tts');
      await appDownloadDir.create(recursive: true);
      return appDownloadDir;
    }

    final appDir = await getApplicationDocumentsDirectory();
    final fallbackDir = Directory('${appDir.path}/Adamson-tts');
    await fallbackDir.create(recursive: true);
    return fallbackDir;
  }

  String _saveLocationLabel() {
    if (Platform.isAndroid) {
      return 'Internal storage > Download > Adamson-tts';
    }
    if (Platform.isIOS) {
      return 'Files app > On My iPhone > Adamson-tts';
    }
    return 'Adamson-tts folder in your Downloads location';
  }
  ...
  Future<void> _exportAudio() async {
    ...
    try {
      ...
      final wavPath = '${dir.path}/adamson_tts_$timestamp.wav';
      ...
      final location = _saveLocationLabel();
      _showSnack('Saved in $location');
      setState(() => _status = 'WAV saved: ${wavFile.path}');
    } catch (e) {
      ...
      body: ListView(
        ...
        children: [
          ...
          Container(
            ...
            child: const Text(
              'Tip: WAV files are saved in Internal storage > Download > Adamson-tts on Android devices.',
            ),
          ),
        ],
      ),
